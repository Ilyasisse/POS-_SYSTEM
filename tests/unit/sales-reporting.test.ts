...
const query = (value: Record<string, unknown>) =>
  reportQuerySchema.parse(value);

test("resolves current, yesterday, and last-seven-day business ranges", () => {
  assert.equal(
    resolveReportRange(query({}), now).start.toISOString(),
    "2026-07-18T04:00:00.000Z",
  );
  assert.equal(
    resolveReportRange(query({ preset: "yesterday" }), now).start.toISOString(),
...
test("resolves Saturday weeks and calendar months", () => {
  assert.equal(
    resolveReportRange(query({ preset: "thisWeek" }), now).start.toISOString(),
    "2026-07-18T04:00:00.000Z",
  );
  assert.equal(
    resolveReportRange(query({ preset: "lastWeek" }), now).start.toISOString(),
    "2026-07-11T04:00:00.000Z",
  );
  assert.equal(
    resolveReportRange(query({ preset: "thisMonth" }), now).start.toISOString(),
    "2026-07-01T04:00:00.000Z",
  );
  assert.equal(
    resolveReportRange(query({ preset: "lastMonth" }), now).start.toISOString(),
...
test("includes the complete end business day for custom ranges", () => {
  const range = resolveReportRange(
    query({ preset: "custom", from: "2026-07-10", to: "2026-07-12" }),
    now,
  );
