...
3. Set the server-only `OPENAI_API_KEY` and `OPENAI_INVOICE_MODEL` variables. The configured model must support image input and structured outputs through the Responses API. Never expose either value through a `NEXT_PUBLIC_` variable.
...
Each upload and explicit retry makes one OpenAI request containing only the uploaded invoice image and extraction instructions. Failed extraction keeps the uploaded delivery pending for a manager retry.
