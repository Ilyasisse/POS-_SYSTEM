...
const auditValue = (value: unknown) =>
  JSON.parse(serializeAuditValue(value)) as Prisma.InputJsonValue;
const businessDate = (instant: Date) =>
  new Date(`${formatBusinessDate(instant)}T00:00:00.000Z`);
...
export async function saveEmploymentProfile(input: {
  userId: string;
  compensationType: CompensationType;
  dailyRate?: Prisma.Decimal.Value | null;
  ...
  const dailyRate =
    input.compensationType === "DAILY" ? money(input.dailyRate ?? 0) : null;
  const monthlySalary =
    input.compensationType === "MONTHLY"
      ? money(input.monthlySalary ?? 0)
      : null;
  if (
    (dailyRate && dailyRate.lte(0)) ||
    (monthlySalary && monthlySalary.lte(0))
  )
    throw new Error("Compensation must be greater than zero.");
  if (input.effectiveTo && input.effectiveTo < input.effectiveFrom)
    throw new Error("Effective end date must be after the start date.");
  const profile = await prisma.$transaction(async (tx) => {
    const { previous, saved } = await tx.employmentProfile
...
export async function saveAttendancePolicy(input: {
  ...
  const policy = await prisma.$transaction(async (tx) => {
    const { previous, saved } = await tx.attendancePolicy
      .findUnique({ where: { id: "default" } })
      .then(async (previous) => ({
        previous,
        saved: await tx.attendancePolicy.upsert({
          where: { id: "default" },
          create: {
            id: "default",
...
export async function createScheduledShift(input: {
  ...
  const shift = await prisma.$transaction(
    async (tx) => {
      const overlap = await tx.scheduledShift.findFirst({
        where: {
      ...
      if (overlap)
        throw new Error("This worker already has an overlapping shift.");
      const created = await tx.scheduledShift.create({
        data: {
          workerId: input.workerId,
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityId: created.id,
          newValue: auditValue(created),
          relatedEntityType: "User",
          relatedEntityId: input.workerId,
        },
      });
      return created;
    },
...
export async function cancelScheduledShift(input: {
  ...
  const shift = await prisma.$transaction(async (tx) => {
    const previous = await tx.scheduledShift.findUnique({
      where: { id: input.shiftId },
    });
    if (!previous || previous.status !== "SCHEDULED")
      throw new Error("Open scheduled shift not found.");
    ...
    await tx.auditLog.create({
      data: {
        actorUserId: input.actorUserId,
        action: "attendance.shift.cancelled",
        entityType: "ScheduledShift",
        entityId: updated.id,
        reason: input.reason,
        previousValue: auditValue(previous),
    ...
    async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          action: `attendance.clock.${input.type.toLowerCase()}`,
  ...
  const record = await prisma.$transaction(
    async (tx) => {
      ...
      if (!shift || shift.status === "CANCELLED")
        throw new Error("Scheduled shift not found.");
      ...
      const events = await tx.clockEvent.findMany({
        where: {
          ...
          occurredAt: {
            gte: new Date(shift.startsAt.getTime() - 6 * 3_600_000),
            lte: new Date(shift.endsAt.getTime() + 12 * 3_600_000),
          },
        },
      ...
      const approvedOvertimeMinutes = Math.max(
        0,
        Math.min(
          input.approvedOvertimeMinutes ?? outcome.overtimeMinutes,
          outcome.overtimeMinutes,
        ),
      );
      const existing = await tx.attendanceRecord.findFirst({
        where: {
          OR: [
            ...
            {
              workerId: shift.workerId,
              businessDate: businessDate(shift.startsAt),
            },
          ],
        },
      });
      const data = {
        ...
        absenceReason:
          input.status === "ABSENT"
            ? input.absenceReason?.trim() || "Not recorded"
            : null,
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityId: saved.id,
          previousValue: existing ? auditValue(existing) : undefined,
          newValue: auditValue(saved),
...
export async function approvePayrollAdjustment(input: {
  adjustmentId: string;
  actorUserId: string;
}) {
  const adjustment = await prisma.$transaction(async (tx) => {
    const previous = await tx.payrollAdjustment.findUnique({
      where: { id: input.adjustmentId },
    });
    if (!previous || previous.approvedAt)
      throw new Error("Pending payroll adjustment not found.");
    const updated = await tx.payrollAdjustment.update({
      where: { id: previous.id },
      data: { approvedAt: new Date(), approvedByUserId: input.actorUserId },
    });
    await tx.auditLog.create({
      data: {
        actorUserId: input.actorUserId,
...
export async function createPayrollRun(input: {
  ...
  const run = await prisma.$transaction(
    async (tx) => {
      ...
      if (overlapping)
        throw new Error("A finalized payroll already overlaps this period.");
      const [profiles, attendance, adjustments] = await Promise.all([
        ...
        tx.attendanceRecord.findMany({
          where: {
            businessDate: { gte: input.periodStart, lte: input.periodEnd },
            status: "PRESENT",
            approvedAt: { not: null },
          },
        }),
      ...
      const lines = profiles.map((profile) => {
        ...
        const additions = workerAdjustments
          .filter((row) =>
            ["BONUS", "CORRECTION", "OVERTIME"].includes(row.type),
          )
          .reduce((sum, row) => sum.plus(row.amount), money(0));
        const deductions = workerAdjustments
          .filter((row) => ["ADVANCE", "ABSENCE_DEDUCTION"].includes(row.type))
          .reduce((sum, row) => sum.plus(row.amount), money(0));
        const overtimeMinutes = workerAttendance.reduce(
          (sum, row) => sum + row.approvedOvertimeMinutes,
          0,
        );
        const result = calculatePayrollLine({
          ...
          approvedAttendanceDays: workerAttendance.length,
        ...
        return {
          ...
          attendanceDays: workerAttendance.length,
          overtimeMinutes,
          snapshot: {
            employee: profile.user.fullName,
            dailyRate: profile.dailyRate?.toFixed(2) ?? null,
            monthlySalary: profile.monthlySalary?.toFixed(2) ?? null,
            adjustmentIds: workerAdjustments.map((row) => row.id),
          },
        };
      });
...
export async function transitionPayrollRun(input: {
  runId: string;
  action: "APPROVE" | "FINALIZE" | "VOID";
  actorUserId: string;
  reason?: string;
}) {
  const run = await prisma.$transaction(
    async (tx) => {
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext('payroll-finalization'))`;
      const previous = await tx.payrollRun.findUnique({
        where: { id: input.runId },
      });
      if (!previous) throw new Error("Payroll run not found.");
      if (input.action === "APPROVE" && previous.status !== "DRAFT")
        throw new Error("Only a draft payroll can be approved.");
      if (input.action === "FINALIZE" && previous.status !== "APPROVED")
        throw new Error("Only an approved payroll can be finalized.");
      if (input.action === "VOID" && previous.status === "VOIDED")
      ...
      if (input.action === "FINALIZE") {
        const overlap = await tx.payrollRun.findFirst({
          where: {
            id: { not: previous.id },
            status: "FINALIZED",
            periodStart: { lte: previous.periodEnd },
            periodEnd: { gte: previous.periodStart },
          },
          select: { id: true },
        });
        if (overlap)
          throw new Error("Another finalized payroll overlaps this period.");
      }
          ...
          input.action === "APPROVE"
            ...
            : input.action === "FINALIZE"
              ...
              : {
                  status: "VOIDED",
                  voidedAt: new Date(),
                  voidReason: input.reason?.trim() || "Voided by administrator",
                },
      });
      await tx.auditLog.create({
        data: {
          actorUserId: input.actorUserId,
          action: `payroll.run.${input.action.toLowerCase()}`,
          entityType: "PayrollRun",
          entityId: updated.id,
          reason: input.reason,
          previousValue: auditValue(previous),
