...
export function attendanceOutcome(input: {
  ...
  const lateMinutes = input.clockIn
    ? Math.max(
        0,
        Math.floor(
          (input.clockIn.getTime() - input.scheduledStart.getTime()) / 60_000,
        ) - input.graceMinutes,
      )
    : 0;
  return {
    ...
    overtimeMinutes: Math.max(
      0,
      workedMinutes - input.overtimeThresholdMinutes,
    ),
  };
}
...
export function calculatePayrollLine(input: {
  ...
  additions?: Prisma.Decimal.Value;
  deductions?: Prisma.Decimal.Value;
  overtimeHourlyRate?: Prisma.Decimal.Value;
}) {
  const zero = money(0);
  const basePay =
    input.compensationType === "DAILY"
      ? money(input.dailyRate ?? 0).times(input.approvedAttendanceDays)
      : money(input.monthlySalary ?? 0);
  const overtimePay = money(
    input.overtimeHourlyRate ??
      (input.compensationType === "DAILY"
        ? money(input.dailyRate ?? 0).div(8)
        : money(input.monthlySalary ?? 0)
            .div(30)
            .div(8)),
  )
    .times(input.approvedOvertimeMinutes)
    .div(60);
  ...
  return {
    ...
    additions,
    deductions,
    netPay: basePay.plus(overtimePay).plus(additions).minus(deductions),
  };
}
