...
import {
  ...
  fundingFor,
  roundMoney,
  validateSavingsDepositAmount,
} from "./money";
import {
  buildDailyCashPaidBreakdown,
  calculatePaidBreakdownTotals,
} from "./paid-breakdown";
import { resolveDailySalaryRate } from "./salary-rates";
import { summarizeDailyCashShiftCash } from "./shift-cash";
import {
  selectDailyCashObligations,
  validateSupplierObligationPaymentAmount,
} from "./supplier-obligations";
...
const number = (value: { toString(): string } | number | null | undefined) =>
...
async function materializeDay(tx: Tx, dateKey: string) {
  ...
  const rates = await tx.dailySalaryRate.findMany({
    orderBy: { effectiveBusinessDate: "desc" },
  });
...
async function currentState(tx: Tx, dateKey: string) {
  ...
  const waiterBalanceDate = businessDateKeyToDatabaseDate(waiterBalanceDateKey);
  ...
  const savingsAccountBalance = roundMoney(
    number(savingsAggregate._sum.amount),
  );
  const salaryPaid =
    day.salaryPaidAt !== null || number(day.salaryAmount) === 0;
  const paidRevenueFunded =
    number(day.salaryRevenueFunded) +
    day.manualExpenses.reduce(
      (sum, row) => sum + number(row.revenueFunded),
      0,
    ) +
    day.supplierPayments.reduce(
      (sum, row) => sum + number(row.revenueFunded),
      0,
    ) +
    day.supplyPayments.reduce((sum, row) => sum + number(row.revenueFunded), 0);
  const paidSavingsFunded =
    number(day.salarySavingsFunded) +
    day.manualExpenses.reduce(
      (sum, row) => sum + number(row.savingsFunded),
      0,
    ) +
    day.supplierPayments.reduce(
      (sum, row) => sum + number(row.savingsFunded),
    ...
    day.supplyPayments.reduce((sum, row) => sum + number(row.savingsFunded), 0);
  const unpaidRequired =
    (salaryPaid ? 0 : number(day.salaryAmount)) +
    obligations.reduce((sum, row) => sum + row.amount, 0) +
    supplyObligations.reduce((sum, row) => sum + row.amount, 0);
    ...
    supplierPayments: day.supplierPayments.map((row) => ({
      ...
      invoiceNumber: (() => {
        const invoiceNumbers = [
          ...new Set(
            row.supplierPayment.allocations.map((allocation) =>
  ...
  const paidBreakdownTotals = calculatePaidBreakdownTotals(
    endDayCash,
    paidBreakdownRows,
  );
  const fingerprint = dailyCashFingerprint({
    ...
    salary: [
      number(day.salaryAmount),
      day.salaryOverridden,
      day.salaryPaidAt?.toISOString() ?? null,
      number(day.salaryRevenueFunded),
      number(day.salarySavingsFunded),
    ],
    manual: day.manualExpenses.map((row) => [
      row.id,
      row.description,
      row.note,
      number(row.amount),
      number(row.revenueFunded),
      number(row.savingsFunded),
    ]),
    payments: day.supplierPayments.map((row) => [
      row.id,
      row.supplierPaymentId,
      number(row.amount),
      number(row.revenueFunded),
      number(row.savingsFunded),
    ]),
    ...
    supplyObligations: supplyObligations.map((row) => [
      row.supplyDayId,
  ...
  const status: DailyCashStatus = locked
    ? "LOCKED"
    : !day.finalizedAt
      ? "OPEN"
      : day.finalizationFingerprint === fingerprint
        ? "FINALIZED"
        : "NEEDS_REVIEW";
  return {
    day,
...
async function mutate<T>(
  ...
  const valid = assertDailyCashBusinessDate(dateKey, now);
  if (isDailyCashLocked(valid, now))
    throw new Error("This Daily Cash day is permanently locked.");
...
export async function createDailySalaryRate(input: {
  amount: number;
  effectiveDate: string;
  userId: string;
}) {
  if (!Number.isFinite(input.amount) || input.amount < 0)
    throw new Error("Salary amount must be zero or more.");
  const date = assertDailyCashBusinessDate(input.effectiveDate);
  return prisma.dailySalaryRate.upsert({
    where: { effectiveBusinessDate: businessDateKeyToDatabaseDate(date) },
    create: {
      amount: input.amount,
      effectiveBusinessDate: businessDateKeyToDatabaseDate(date),
      createdByUserId: input.userId,
    },
    update: { amount: input.amount, createdByUserId: input.userId },
  });
}

export async function overrideDailySalary(input: {
  dateKey: string;
  amount: number;
  userId: string;
}) {
  ...
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    ...
    await tx.dailyCashDay.update({
      ...
      data: {
        ...
        salaryOverriddenAt: new Date(),
        salaryOverriddenByUserId: input.userId,
      },
    });
  });
}

export async function payDailySalary(input: {
  dateKey: string;
  userId: string;
  ...
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    ...
    if (funding.savingsFunded > 0 && !input.confirmSavings)
      return {
        ok: false,
        code: "SAVINGS_CONFIRMATION_REQUIRED",
        savingsAmount: funding.savingsFunded.toFixed(2),
      };
    await tx.dailyCashDay.update({
      where: { id: state.day.id },
      data: {
        salaryPaidAt: new Date(),
        salaryPaidByUserId: input.userId,
        salaryRevenueFunded: funding.revenueFunded,
        salarySavingsFunded: funding.savingsFunded,
      },
    });
    return { ok: true };
  });
...
export async function createManualExpense(input: {
  ...
  if (
    ...
    throw new Error("Enter an expense name and positive amount.");
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    ...
    await tx.dailyCashManualExpense.create({
      data: {
        dailyCashDayId: state.day.id,
        description: input.description.trim(),
        amount: input.amount,
        note: input.note?.trim() || null,
        revenueFunded: funding.revenueFunded,
        savingsFunded: funding.savingsFunded,
        createdByUserId: input.userId,
      },
    });
    return { ok: true };
...
export async function deleteManualExpense(input: {
  dateKey: string;
  id: string;
}) {
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    const result = await tx.dailyCashManualExpense.deleteMany({
      where: { id: input.id, dailyCashDayId: state.day.id },
    });
    if (result.count !== 1) throw new Error("One-time expense not found.");
  });
}

export async function undoDailyCashSupplierPayment(input: {
  dateKey: string;
  id: string;
}) {
  const now = new Date();
  const dateKey = assertDailyCashBusinessDate(input.dateKey, now);
  if (isDailyCashLocked(dateKey, now))
    throw new Error("This Daily Cash day is permanently locked.");
  ...
  if (!linkedPayment)
    throw new Error("Supplier payment not found for this business day.");
...
export async function payDailyCashObligation(input: {
  dateKey: string;
  billId: string;
  ...
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    const obligation = state.obligations.find(
      (row) =>
        ...
        row.installmentId === (input.installmentId || null),
    );
    if (!obligation)
      return {
        ok: false,
        code: "STALE_OBLIGATION",
        message: "This supplier obligation is no longer available.",
      };
    const amount = validateSupplierObligationPaymentAmount(
      input.amount,
      obligation.amount,
    ...
    const result = await recordSupplierPaymentInTransaction(tx, {
      supplierId: obligation.supplierId,
      preferredBillId: obligation.billId,
      preferredInstallmentId: obligation.installmentId,
      recordedByUserId: input.userId,
      amount,
      paymentMethod: "DAILY_CASH",
      allowOverpayment: input.allowOverpayment,
    });
    await tx.dailyCashSupplierPayment.create({
      data: {
        dailyCashDayId: state.day.id,
...
export async function payDailyCashSupplierAdvance(input: {
  dateKey: string;
  supplierId: string;
  amount: number;
  userId: string;
...
export async function payDailyCashSupply(input: {
  dateKey: string;
  supplyDayId: string;
  amount: number;
  userId: string;
  ...
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    const obligation = state.supplyObligations.find(
      (row) => row.supplyDayId === input.supplyDayId,
    );
    if (!obligation)
      return {
        ok: false,
        code: "STALE_OBLIGATION",
        message: "This supply balance is no longer available.",
      };
    const amount = validateSupplierObligationPaymentAmount(
      input.amount,
      obligation.amount,
    ...
    await tx.dailyCashSupplyPayment.create({
      data: {
        dailyCashDayId: state.day.id,
        supplyDayId: obligation.supplyDayId,
        amount,
        revenueFunded: funding.revenueFunded,
        savingsFunded: funding.savingsFunded,
        recordedByUserId: input.userId,
...
export async function undoDailyCashSupplyPayment(input: {
  dateKey: string;
  id: string;
}) {
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    const result = await tx.dailyCashSupplyPayment.deleteMany({
      where: { id: input.id, dailyCashDayId: state.day.id },
    });
  ...
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    ...
    try {
      ...
      return {
        ...
        code: "VALIDATION_ERROR",
        message:
          error instanceof Error ? error.message : "Invalid savings amount.",
      };
    }
    ...
    if (note && note.length > 500) {
      return {
        ok: false,
        code: "VALIDATION_ERROR",
        message: "Savings notes cannot exceed 500 characters.",
      };
    }
...
export async function undoDailyCashSavingsDeposit(input: {
  dateKey: string;
  id: string;
}) {
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    const result = await tx.dailyCashSavingsDeposit.deleteMany({
      where: { id: input.id, dailyCashDayId: state.day.id },
    });
...
export async function finalizeDailyCash(input: {
  dateKey: string;
  userId: string;
}) {
  return mutate(input.dateKey, new Date(), async (tx, state) => {
    await tx.dailyCashDay.update({
      ...
      data: {
        ...
        finalizationFingerprint: state.fingerprint,
        finalizedRevenue: state.endDayCash,
        finalizedPaidExpenses:
          state.paidRevenueFunded + state.paidSavingsFunded,
        finalizedUnpaidRequired: state.unpaidRequired,
        finalizedRemainingCash: state.summary.projectedRemaining,
        finalizedSavingsUsed: state.summary.savingsUsed,
        finalizedAdditionalSavingsRequired:
          state.summary.additionalSavingsRequired,
      },
    });
  });
}
