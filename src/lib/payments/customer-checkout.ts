...
export async function expireCustomerCheckout(checkoutId: string) {
  await prisma.customerCheckout.updateMany({
    where: {
      id: checkoutId,
      status: {
        in: [CustomerCheckoutStatus.PENDING, CustomerCheckoutStatus.REVIEW],
      },
  ...
  try {
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        const checkout = await tx.customerCheckout.findUnique({
          where: { id: checkoutId },
        });
        ...
        const modifiers = lines.flatMap((line, index) =>
          line.modifiers
            .filter(
              (modifier) => !modifier.optionId.startsWith("placeholder__"),
            )
            .map((modifier) => ({
        ...
        if (modifiers.length)
          await tx.orderItemModifier.createMany({ data: modifiers });
    ...
    console.error(
      "Paid customer checkout requires staff help:",
      checkoutId,
      error,
    );
    await prisma.customerCheckout.updateMany({
...
export async function assignCustomerCheckoutReceipt(input: {
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      if (!receipt || !checkout)
        throw new Error("Receipt or checkout not found.");
      if (!claimable.includes(checkout.status))
        throw new Error("Checkout is already settled.");
      if (
      ...
      if (
        receipt.transactionAt.getTime() <
        checkout.createdAt.getTime() - 1000
      ) {
      ...
      if (claimed.count !== 1)
        throw new Error("Receipt was assigned elsewhere.");
...
export async function retryCustomerCheckoutPayment(checkoutId: string) {
  ...
  if (
    ...
    (checkout.status !== CustomerCheckoutStatus.PENDING &&
      checkout.status !== CustomerCheckoutStatus.REVIEW) ||
