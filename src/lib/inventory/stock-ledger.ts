...
import { Prisma, type StockEventType } from "@prisma/client";
...
function signedQuantity(value: Prisma.Decimal | string | number) {
  const quantity = new Prisma.Decimal(value).toDecimalPlaces(
    6,
  ...
  const item = isProduct
    ? await (async () => {
        ...
        if (updated.count !== 1)
          throw new Error(
            delta.isNegative()
              ? "Insufficient inventory for this stock event."
              : "Inventory target not found.",
          );
...
function appendStockEventsInOrder(
  ...
  return mutations.reduce<
    Promise<Awaited<ReturnType<typeof appendStockEvent>>[]>
  >(
...
export async function recordSupplierInvoiceReceipts(
  ...
  const receiptPlan = items.reduce<{
    ...
    (plan, item) => {
      ...
      if (catalog.inventorySupply) {
        const conversion = catalog.inventorySupply.purchaseUnitConversions.find(
          (candidate) =>
            candidate.isActive &&
            candidate.purchaseUnit.trim().toLowerCase() ===
              item.itemUnit.trim().toLowerCase(),
        );
        if (
          !conversion ||
        ...
        plan.mutations.push({
          ...
          type: "RECEIPT",
          quantityDelta: convertPurchaseQuantity(
            item.quantity,
            conversion.canonicalQuantity,
          ),
...
export async function updateProductStandardCost(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const cost =
      input.cost == null
        ? null
        : decimalCost(input.cost, "Product standard cost");
    const product = await tx.product.update({
      where: { id: input.productId },
      data: {
        cost,
        standardCostUpdatedAt: new Date(),
        standardCostUpdatedByUserId: input.actorUserId,
      },
    });
...
export async function updateSupplyStandardCost(input: {
  ...
  return prisma.$transaction(async (tx) => {
    const current = await tx.inventorySupply.findUniqueOrThrow({
      where: { id: input.supplyId },
      select: { standardUnitCost: true },
    });
    const cost =
      input.cost == null
        ? null
        : decimalCost(input.cost, "Supply standard cost");
...
export async function createRecipeVersion(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (supplies.length !== input.ingredients.length)
      throw new Error("Every recipe ingredient must reference a supply.");
    const costs = new Map(
      supplies.map((supply) => [supply.id, supply.standardUnitCost]),
    );
    const cost = calculateRecipeStandardCost(
      input.yieldQuantity,
      input.ingredients.map((ingredient) => ({
        quantity: ingredient.quantity,
        standardUnitCost: costs.get(ingredient.supplyId) ?? null,
      })),
    );
    ...
    return tx.recipeVersion.create({
      data: {
        ...
        yieldQuantity: positiveDecimalQuantity(
          input.yieldQuantity,
          "Recipe yield",
        ),
...
export async function submitInventoryCount(
  ...
  if (result.count !== 1)
    throw new Error("Only a non-empty draft count can be submitted.");
}
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (!session || session.status !== "SUBMITTED")
        throw new Error("Only a submitted count can be approved.");
      await session.lines.reduce<Promise<void>>(
        (pendingLine, line) =>
          pendingLine.then(async () => {
