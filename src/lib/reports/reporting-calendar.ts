...
export function parseBusinessDate(value: string): CafeDate | null {
  ...
  return validated.getUTCFullYear() === result.year &&
    validated.getUTCMonth() + 1 === result.month &&
    validated.getUTCDate() === result.day
    ? result
    : null;
}
...
export function getReportingWeekRange(anchor: Date = new Date()): ReportRange {
  ...
  const startDate = addCafeDays(
    date,
    -((weekday - REPORTING_WEEK_START_DAY + 7) % 7),
  );
  return {
    start: utcFromCafe(startDate, BUSINESS_DAY_START_MINUTE),
    end: utcFromCafe(addCafeDays(startDate, 7), BUSINESS_DAY_END_MINUTE),
  };
}
...
export function getReportingMonthRange(anchor: Date = new Date()): ReportRange {
  ...
  return {
    start: utcFromCafe(startDate, BUSINESS_DAY_START_MINUTE),
    end: utcFromCafe(endDate, BUSINESS_DAY_START_MINUTE),
  };
}
...
export function getComparisonRanges(anchor: Date = new Date()) {
  ...
  return {
    current,
    yesterday: shiftRange(current, -1),
    sameWeekdayLastWeek: shiftRange(current, -7),
  };
}
