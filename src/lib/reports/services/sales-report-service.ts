...
function addRow(
  ...
  key: string,
  name: string,
  input: { qty: number; gross: Prisma.Decimal; cost: Prisma.Decimal | null },
) {
...
export async function getSalesReport(range: ReportRange, query: ReportQuery) {
  ...
  for (const order of orders) {
    for (const payment of order.payments)
      paymentTotals.set(
        payment.method,
        (paymentTotals.get(payment.method) ?? zero()).plus(payment.amountPaid),
      );
    let orderGross = zero();
    ...
    for (const item of order.orderItems) {
      ...
      addRow(productRows, item.productId, item.productName, {
        qty: item.qty,
        gross: item.lineTotal,
        cost: item.unitCostSnapshot,
      });
      addRow(
        categoryRows,
        item.product.category.id,
        item.product.category.name,
        { qty: item.qty, gross: item.lineTotal, cost: item.unitCostSnapshot },
      );
    }
  ...
  const profit =
    totalLines > 0 && costCoveredLines === totalLines
      ...
      : null;
  const mapRows = (rows: Map<string, SalesRow>) =>
    [...rows.entries()]
      .map(([id, row]) => ({
        id,
        name: row.name,
        ...
        grossProfit:
          row.missingCostLines === 0
            ? row.grossSales.minus(row.cogs).toFixed(2)
            : null,
  ...
  return {
    ...
    summary: {
      ...
      complimentary: complimentary.toFixed(2),
      staffMeals: staffMeals.toFixed(2),
      ...
      costCoveragePercent: serialize(
        ratioPercent(costCoveredLines, totalLines),
      ),
      costCoveredLines,
      totalLines,
    },
