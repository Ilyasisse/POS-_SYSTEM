...
import {
  ...
  getStaffReport,
  getSupplierReport,
} from "@/lib/reports/services/advanced-report-service";
...
export function flattenReport(data: unknown) {
  const rows: Array<Record<string, string>> = [];
  const walk = (value: unknown, prefix = "") => {
    if (Array.isArray(value)) {
      value.forEach((item, index) =>
        walk(item, `${prefix}${prefix ? "." : ""}${index}`),
      );
      return;
    }
    if (
      ...
      Object.entries(value as Record<string, unknown>).forEach(([key, child]) =>
        walk(child, `${prefix}${prefix ? "." : ""}${key}`),
      );
      return;
    }
    rows.push({ field: prefix, value: scalar(value) });
  };
  walk(data);
  return rows;
}
...
export async function toPdf(
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    document.fontSize(9);
    rows.forEach((row) =>
      document.text(`${row.field}: ${row.value}`, { width: 520 }),
    );
    document.end();
  });
}
