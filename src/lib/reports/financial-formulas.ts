...
export const netSales = (
  ...
  refunds: DecimalInput,
) => money(gross).minus(discounts).minus(refunds).toDecimalPlaces(2);
export const grossProfit = (net: DecimalInput, cogs: DecimalInput) =>
  money(net).minus(cogs).toDecimalPlaces(2);
export function ratioPercent(
  ...
  return divisor.isZero()
    ? null
    : money(numerator).dividedBy(divisor).times(100).toDecimalPlaces(2);
}
export function averageOrderValue(net: DecimalInput, count: number) {
  return Number.isInteger(count) && count > 0
    ? money(net).dividedBy(count).toDecimalPlaces(2)
    : null;
}
export const settlementVariance = (
  handedIn: DecimalInput,
  expected: DecimalInput,
) => money(handedIn).minus(expected).toDecimalPlaces(2);
export function expectedStock(input: {
...
export function netProfit(input: {
  ...
  return money(input.netSales)
    .minus(input.cogs)
    .minus(input.labour)
    .minus(input.operatingExpenses)
    .toDecimalPlaces(2);
}
export function breakEvenSales(
  fixedCosts: DecimalInput,
  contributionMarginRatio: DecimalInput,
) {
  const ratio = new Prisma.Decimal(contributionMarginRatio);
  return ratio.lte(0)
    ? null
    : money(fixedCosts).dividedBy(ratio).toDecimalPlaces(2);
}
