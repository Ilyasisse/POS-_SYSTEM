...
const links = [
  ...
  ["No sales", "/admin/reports/unsold-products"],
] as const;
...
export async function SalesReportPage({
  ...
  const parsed = reportQuerySchema.parse({
    ...normalized,
    preset: normalized.preset ?? defaultPreset,
  });
    ...
    <AdminPage title={title} description={description}>
      ...
      <Card className="p-4">
        ...
        <p className="mt-3 text-xs text-slate-500">
          ...
          –{" "}
          {new Date(report.period.end).toLocaleString("en-US", {
            timeZone: report.period.timezone,
          })}
        </p>
      </Card>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        {canSeeFinancials ? (
          <MetricCard
            ...
            value={money(report.summary.grossProfit)}
            helper={`Cost coverage ${report.summary.costCoveragePercent ?? "0.00"}%`}
          />
        ) : null}
      </section>
      ...
      {focus === "orders" ? (
        <Card className="p-5">
          <h2 className="mb-4 text-lg font-black">Paid orders</h2>
          <Table>
            <TableHeader>
              <TableRow>
