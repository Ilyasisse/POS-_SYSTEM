...
type Line = { id: string; payerName: string; payerPhone: string; amount: string; tipAmount: string };
type RequestLine = { id: string; payerName: string; payerPhone: string; amount: number; tipAmount: number; totalAmount: number; tipRecipientName: string | null; paidAmount: number; remainingAmount: number; status: string };
type Receipt = {
  ...
  rawMessage: string;
  assignment: { paymentRequestId: string; payerName: string; payerPhone: string; expectedAmount: number; billAmount: number; tipAmount: number; tipRecipientName: string | null; table: { id: string; name: string } } | null;
};
...
export default function CashierPaymentDialog({ tableId, tableName, amountDue }: { tableId: string; tableName: string; amountDue: number }) {
  ...
  const [lines, setLines] = useState<Line[]>([{ id: crypto.randomUUID(), payerName: "", payerPhone: "", amount: amountDue.toFixed(2), tipAmount: "0.00" }]);
  ...
  const tips = useMemo(() => lines.reduce((sum, line) => sum + (Number(line.tipAmount) || 0), 0), [lines]);
  ...
  async function startChecks() {
    ...
    try {
      const response = await fetch("/api/cashier/payment-requests", {
        ...
        body: JSON.stringify({ batchKey: key, tableId, method, payLater, lines: lines.map((line) => ({ payerName: line.payerName, payerPhone: line.payerPhone, amount: Number(line.amount), tipAmount: Number(line.tipAmount) })) }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Could not start payment checks.");
      setBatchKey(key);
      setRequests((data.requests ?? []).map((item: RequestLine) => ({ ...item, paidAmount: 0, remainingAmount: item.totalAmount })));
      ...
      <DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto rounded-[2rem] p-6 sm:p-8">
        ...
        {!batchKey ? (
          <div className="space-y-5">
            ...
            <div className="space-y-3">{lines.map((line, index) => <div key={line.id} className="grid gap-2 rounded-2xl border p-3 sm:grid-cols-[1fr_1fr_7rem_7rem_auto]"><Input aria-label={`Payer ${index + 1} name`} placeholder="Name" value={line.payerName} onChange={(event) => updateLine(line.id, "payerName", event.target.value)} /><Input aria-label={`Payer ${index + 1} phone`} placeholder="Phone number" value={line.payerPhone} onChange={(event) => updateLine(line.id, "payerPhone", event.target.value)} /><Input aria-label={`Payer ${index + 1} bill amount`} type="number" min="0.01" step="0.01" value={line.amount} onChange={(event) => updateLine(line.id, "amount", event.target.value)} /><Input aria-label={`Payer ${index + 1} tip amount`} title="Tip" placeholder="Tip" type="number" min="0" step="0.01" value={line.tipAmount} onChange={(event) => updateLine(line.id, "tipAmount", event.target.value)} /><Button type="button" variant="outline" disabled={lines.length === 1} onClick={() => setLines((current) => current.filter((item) => item.id !== line.id))}>Remove</Button></div>)}</div>
            <Button type="button" variant="outline" onClick={() => setLines((current) => [...current, { id: crypto.randomUUID(), payerName: "", payerPhone: "", amount: remaining ? remaining.toFixed(2) : "", tipAmount: "0.00" }])}>+ Add another payer</Button>
            <div className={`rounded-2xl border p-4 ${Math.abs(remaining) < 0.001 ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"}`}><div className="flex justify-between"><span>Table balance</span><strong>{money(amountDue)}</strong></div><div className="mt-1 flex justify-between"><span>Entered bill payments</span><strong>{money(entered)}</strong></div><div className="mt-1 flex justify-between"><span>Tips</span><strong>{money(tips)}</strong></div><div className="mt-1 flex justify-between"><span>Total to collect</span><strong>{money(entered + tips)}</strong></div><div className="mt-1 flex justify-between"><span>Bill remaining</span><strong>{money(remaining)}</strong></div></div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid gap-3 md:grid-cols-2">{requests.map((request) => <button key={request.id} type="button" onClick={() => setSelectedRequestId(request.id)} className={`rounded-2xl border p-4 text-left ${request.id === selectedRequestId ? "border-blue-500 bg-blue-50 ring-2 ring-blue-100" : "hover:bg-muted/40"}`}><span className="font-semibold">{request.payerName} · {request.payerPhone}</span><span className="mt-2 block text-sm">Bill {money(request.amount)} + tip {money(request.tipAmount)} = {money(request.totalAmount)}</span><span className="mt-1 block text-sm">Paid {money(request.paidAmount)} · Remaining {money(request.remainingAmount)}</span>{request.tipAmount > 0 ? <span className="mt-1 block text-xs text-muted-foreground">Tip recipient: {request.tipRecipientName ?? "unassigned"}</span> : null}<span className={`mt-2 inline-flex rounded-full px-3 py-1 text-xs font-bold ${request.status === "MATCHED" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{request.status.replaceAll("_", " ")}</span></button>)}</div>
