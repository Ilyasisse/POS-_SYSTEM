...
export default async function PayrollPage() {
  ...
  const finalized = runs.filter((run) => run.status === "FINALIZED");
  const paid = finalized.reduce(
    (sum, run) =>
      sum +
      run.lines.reduce((lineSum, line) => lineSum + Number(line.netPay), 0),
    0,
  );
  return (
    <AdminPage
    ...
    >
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <MetricCard
          ...
          value={adjustments.filter((row) => !row.approvedAt).length}
        />
