...
const optionalDate = (value: FormDataEntryValue | null) =>
  value ? z.coerce.date().parse(value) : null;
const optionalMoney = (value: FormDataEntryValue | null) =>
  value ? z.coerce.number().positive().parse(value) : null;
const refresh = () => {
...
export async function saveEmploymentAction(formData: FormData) {
  ...
  const compensationType = z
    .enum(["DAILY", "MONTHLY"])
    .parse(formData.get("compensationType"));
  await saveEmploymentProfile({
    userId: id.parse(formData.get("userId")),
    compensationType,
    dailyRate:
      compensationType === "DAILY"
        ? optionalMoney(formData.get("dailyRate"))
        : null,
    monthlySalary:
      compensationType === "MONTHLY"
        ? optionalMoney(formData.get("monthlySalary"))
        : null,
...
export async function saveAttendancePolicyAction(formData: FormData) {
  ...
  await saveAttendancePolicy({
    shiftMinutes: z.coerce
      ...
      .min(60)
      .max(1440)
      .parse(formData.get("shiftMinutes")),
    graceMinutes: z.coerce
      .number()
      .int()
      .min(0)
      .max(120)
      .parse(formData.get("graceMinutes")),
...
export async function createScheduleAction(formData: FormData) {
  ...
  await createScheduledShift({
    ...
    station: z
      ...
      .nullable()
      .parse(formData.get("station") || null),
    actorUserId: actor.id,
  });
  refresh();
}
...
export async function cancelScheduleAction(formData: FormData) {
  ...
  await cancelScheduledShift({
    ...
    reason: z
      ...
      .min(3)
      .max(250)
      .parse(formData.get("reason") ?? "Cancelled by schedule manager"),
  });
  refresh();
}
...
export async function approveAttendanceAction(formData: FormData) {
  ...
  await approveAttendance({
    ...
    status: z
      ...
      .parse(formData.get("status")),
    approvedOvertimeMinutes: z.coerce
      .number()
      .int()
      .min(0)
      .max(1440)
      .parse(formData.get("approvedOvertimeMinutes") || 0),
...
export async function createPayrollAdjustmentAction(formData: FormData) {
  ...
  await createPayrollAdjustment({
    ...
    type: z
      .enum(["BONUS", "ADVANCE", "ABSENCE_DEDUCTION", "CORRECTION", "OVERTIME"])
...
export async function approvePayrollAdjustmentAction(formData: FormData) {
  const actor = await requirePermission(PERMISSIONS.PAYROLL_MANAGE);
  await approvePayrollAdjustment({
    adjustmentId: id.parse(formData.get("adjustmentId")),
    actorUserId: actor.id,
  });
  refresh();
}
