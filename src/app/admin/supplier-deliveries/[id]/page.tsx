...
export default async function SupplierDeliveryDetailPage({
  ...
  const targets = [
    ...products.map((row) => ({
      ...
      label: `Product Â· ${row.name}`,
    })),
    ...supplies.map((row) => ({
      value: `supply:${row.id}`,
      label: `Supply Â· ${row.name} (${row.unit})`,
    })),
  ];

  return (
    <AdminPage
      title={`${delivery.supplier.name} delivery`}
      description={`Submitted ${delivery.submittedAt.toLocaleString()} by ${delivery.uploadedByEmail}`}
    >
      <div className="flex flex-wrap gap-2">
        ...
        {delivery.bill ? (
          <ToneBadge tone={delivery.bill.status === "PAID" ? "green" : "amber"}>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(20rem,0.8fr)_minmax(0,1.5fr)]">
        <Card className="p-4">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="relative block aspect-[4/5] overflow-hidden rounded-2xl bg-slate-100"
          >
            <Image
              src={receiptUrl}
              alt="Supplier receipt"
              fill
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <dd className="font-bold">{delivery.invoiceNumber || "--"}</dd>
            </div>
            <div>
              <dt className="text-slate-500">Total</dt>
              <dd className="font-bold">{money(delivery.totalAmount)}</dd>
            </div>
            <div>
              <dt className="text-slate-500">Receipt date</dt>
              <dd className="font-bold">
                {delivery.receiptDate?.toLocaleDateString() || "--"}
              </dd>
            </div>
            <div>
              <dt className="text-slate-500">Inventory updated</dt>
              <dd className="font-bold">
        ...
        <Card className="p-4">
          <h2 className="mb-4 font-black">Verification</h2>
          <DeliveryVerificationForm
            deliveryId={delivery.id}
            status={delivery.status}
            extractionError={delivery.aiError}
            extractedText={delivery.extractedText || ""}
            ...
            items={delivery.items.map((item) => ({
              id: item.id,
              description: item.aiItemName,
              matchedTarget: item.productId
                ? `product:${item.productId}`
                : item.inventorySupplyId
                  ? `supply:${item.inventorySupplyId}`
                  : "",
      ...
      {delivery.status === "VERIFIED" && delivery.items.length ? (
        <Card className="overflow-hidden">
          <div className="border-b border-slate-200 px-4 py-3">
            <h2 className="font-black">Accepted invoice items</h2>
          </div>
            ...
            <Table className="min-w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th className="px-4 py-3">Inventory match</th>
              ...
              <tbody>
                {delivery.items.map((item) => (
                  <tr key={item.id} className="border-t border-slate-100">
                    <td className="px-4 py-3 font-bold">{item.aiItemName}</td>
      ...
      <Card className="p-4">
        {delivery.reviewedText ? (
          <div>
            ...
            <pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-50 p-4 text-sm text-slate-800">
              {delivery.reviewedText}
            </pre>
          </div>
        ) : null}
        <details
          className={delivery.reviewedText ? "mt-4" : ""}
          open={!delivery.reviewedText}
        >
          <summary className="cursor-pointer font-black">
            Original extracted text
          </summary>
          {delivery.extractedText ? (
            <pre className="mt-3 max-h-96 overflow-auto whitespace-pre rounded-xl bg-slate-950 p-4 text-xs text-slate-100">
              {delivery.extractedText}
            </pre>
          ) : (
            <p className="mt-3 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">
              {delivery.aiError || "No invoice transcription is available."}
            </p>
          )}
        </details>
      </Card>
