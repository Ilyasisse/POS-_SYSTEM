...
import { createManualExpenseAction, deleteManualExpenseAction, finalizeDailyCashAction, overrideSalaryAction, paySalaryAction, paySupplierObligationAction } from "./actions";
...
function TodayCashBreakdown({ data, date }: { data: DailyCashData; date: string }) {
  return <Card>
    ...
    <CardContent className="space-y-5 px-0">
      <Table>
        <TableHeader><TableRow><TableHead>Type</TableHead><TableHead>Where the money went</TableHead><TableHead>Paid at</TableHead><TableHead className="text-right">From revenue</TableHead><TableHead className="text-right">From savings</TableHead><TableHead className="text-right">Total</TableHead>{!data.locked ? <TableHead><span className="sr-only">Actions</span></TableHead> : null}</TableRow></TableHeader>
        <TableBody>
      ...
      <div className="ml-auto grid max-w-md gap-2 px-6 text-sm">
        <div className="flex items-center justify-between gap-6"><span className="text-muted-foreground">Reported revenue</span><span className="font-medium tabular-nums">+{money(data.revenue)}</span></div>
...
export default async function DailyCashPage({ searchParams }: { searchParams?: Promise<{ date?: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
    <PageHeader eyebrow="Operations" title="Daily cash" description="Use waiter revenue to pay today’s salary, all pending supplier obligations, and one-time expenses." />
...
function DailyCashContent({ data, date }: { data: DailyCashData; date: string }) {
  ...
  return <>
    ...
    {data.missingWaiters.length ? <Alert><AlertCircle /><AlertTitle>Revenue is incomplete</AlertTitle><AlertDescription>Missing waiter settlements: {data.missingWaiters.map((row) => row.fullName).join(", ")}. You may still record payments.</AlertDescription></Alert> : null}
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <Metric label="Reported revenue" value={money(data.revenue)} help="From Waiter Balances" />
      <Metric label="Paid expenses" value={money(paidExpenses)} help="Salary, manual, and supplier payments" />
      <Metric label="Unpaid required" value={money(data.unpaidRequired)} help="Salary and eligible invoices" />
