...
function tone(status: SupplierOrderRunStatus) {
  ...
  if (status === "COLLECTING" || status === "FINALIZING")
...
export default async function SupplierOrderScheduleDetailPage({
  ...
  const [schedule, suppliers, employees] = await Promise.all([
    ...
    prisma.staff.findMany({
      where: { isActive: true, role: { not: "SUPPLIER" } },
  ...
  const formSendAt =
    schedule.nextSupplierSendAt ?? schedule.firstSupplierSendAt;
            ...
            <tbody>
              {schedule.runs.length ? (
                schedule.runs.map((run) => {
                  ...
                  const noOrder = run.recipients.filter(
                    (recipient) => recipient.status === "NO_ORDER",
                  ).length;
                    ...
                    <tr key={run.id} className="border-t align-top">
                      ...
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">
                          to {dateTime.format(run.supplierSendAt)}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>
                          {responded} ordered · {noOrder} no order
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {pending} missed deadline / pending
                        </div>
                      </TableCell>
                      <TableCell>
                        {run.purchaseOrder ? (
                      ...
                      <TableCell>
                        <ToneBadge tone={tone(run.status)}>
                          {run.status}
                        </ToneBadge>
                        {run.failureReason ? (
      ...
      <section>
        ...
        <ScheduleForm
          ...
          defaults={{
            ...
            firstSupplierSendAt: formatDateTimeLocal(
              formSendAt,
              schedule.timeZone,
            ),
            ...
            endAt: schedule.endAt
              ? formatDateTimeLocal(schedule.endAt, schedule.timeZone)
              : "",
            deliveryLeadDays: schedule.deliveryLeadDays,
            employeeIds: schedule.recipients.map(
