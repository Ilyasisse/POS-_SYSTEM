    ...
    <form action={action} className="space-y-6">
      ...
      <Card className="grid gap-5 p-5 lg:grid-cols-2">
          ...
          >
            ...
            {suppliers.map((supplier) => (
              <option key={supplier.id} value={supplier.id}>
                {supplier.name}
                {supplier.phone ? ` (${supplier.phone})` : " (phone missing)"}
              </option>
            ))}
          </NativeSelect>
        </div>
        <div className="grid gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            Use an IANA timezone such as Africa/Nairobi.
          </p>
        </div>
        <div className="grid gap-2">
          ...
          <Input
            ...
            min="0"
            max="365"
            defaultValue={defaults.deliveryLeadDays}
            required
          />
        </div>
      </Card>
      ...
      <Card className="grid gap-5 p-5 lg:grid-cols-2">
        <div className="grid gap-2">
          ...
          <Input
            ...
            name="firstInviteAt"
            type="datetime-local"
            defaultValue={defaults.firstInviteAt}
            required
          />
        </div>
        <div className="grid gap-2">
          <Label htmlFor="supplier-send">
            Supplier send time / response deadline
          </Label>
          <Input
            id="supplier-send"
            name="firstSupplierSendAt"
            type="datetime-local"
            defaultValue={defaults.firstSupplierSendAt}
            required
          />
        </div>
        <div className="grid gap-2">
          ...
          <Input
            ...
            min="5"
            max="10080"
            defaultValue={defaults.reminderIntervalMinutes}
            required
          />
        </div>
        <div className="grid gap-2">
        ...
        <div className="grid gap-2">
          ...
          <NativeSelect
            id="recurrence-unit"
            name="recurrenceUnit"
            defaultValue={defaults.recurrenceUnit}
          >
      ...
      <Card className="p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Each selected employee receives a private link. Invalid or missing
          phone numbers must be fixed in Staff first.
        </p>
      ...
      <div className="flex justify-end">
        <Button type="submit">
          {defaults.id ? "Save schedule" : "Create schedule"}
        </Button>
      </div>
    </form>
  );
