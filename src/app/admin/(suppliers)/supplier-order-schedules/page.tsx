...
export default async function SupplierOrderSchedulesPage({
  ...
  const failed = schedules.filter(
    (schedule) => schedule.runs[0]?.status === "FAILED",
  ).length;
  return (
    <AdminPage
        ...
        <Table>
          ...
          <tbody>
            {schedules.length ? (
              ...
              <tr>
                <TableCell colSpan={6}>
                  No WhatsApp supplier order schedules have been created.
                </TableCell>
              </tr>
            )}
          </tbody>
