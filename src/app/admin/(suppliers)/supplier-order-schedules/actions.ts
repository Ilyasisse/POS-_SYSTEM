...
async function scheduleInput(formData: FormData) {
  ...
  if (
    !Number.isInteger(deliveryLeadDays) ||
    deliveryLeadDays < 0 ||
    deliveryLeadDays > 365
  ) {
  ...
  if (!supplier || !supplier.phone || !normalizeE164Phone(supplier.phone)) {
    throw new Error(
      "Choose an active supplier with a valid E.164 phone number.",
    );
  }
  if (supplier._count.catalogItems === 0)
    throw new Error("The supplier needs at least one active catalog item.");
  if (
    employees.length !== employeeIds.length ||
    employees.some(
      (employee) =>
        !employee.phoneNumber || !normalizeE164Phone(employee.phoneNumber),
    )
  ) {
    throw new Error(
      "Every selected employee must be active and have a valid E.164 phone number.",
    );
  }
  return {
    data: {
...
export async function updateSupplierOrderSchedule(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.supplierOrderScheduleRecipient.deleteMany({
      where: { scheduleId: id },
    });
...
export async function toggleSupplierOrderSchedule(formData: FormData) {
  ...
  if (!schedule.isActive && !schedule.nextInviteAt) {
    throw new Error(
      "This completed one-time schedule cannot be resumed; edit it with new dates instead.",
    );
  }
...
export async function retrySupplierOrderRun(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (retried.count !== 1) {
      throw new Error(
        "Only a failed run on an active schedule can be retried.",
      );
    }
