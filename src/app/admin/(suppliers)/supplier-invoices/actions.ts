...
function draftFromFormData(formData: FormData): SupplierInvoiceDraftInput {
  ...
  if (
    ...
    throw new Error(
      "The installment schedule is incomplete. Refresh and try again.",
    );
  }
...
export async function createManualSupplierInvoiceDraftAction(
  formData: FormData,
) {
...
export async function pauseSupplierInvoiceRecurrenceAction(formData: FormData) {
  const user = await requirePermission(PERMISSIONS.SUPPLIER_MANAGE);
  const invoiceId = text(formData, "invoiceId");
