...
export default function SupplierInvoiceEditor({
  ...
  function updateInstallment(index: number, patch: Partial<EditorInstallment>) {
  ...
  function removeInstallment(index: number) {
    setInstallments((current) => current.filter((_, row) => row !== index));
  }
    ...
    <form ref={formRef} onSubmit={preventSubmit} className="space-y-6">
      ...
      {editable ? (
        <CatalogLinePicker
          ...
          allowCustomLines={
            source === "PURCHASE_ORDER" || source === "LEGACY_UPLOAD"
          }
