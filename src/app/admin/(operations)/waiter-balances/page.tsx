  ...
  switch (status) {
    case "initialized":
      return {
        ...
        description: "The waiter's one-time opening balance is now active.",
        destructive: false,
      };
    case "settlement_saved":
      return {
        title: "Settlement saved",
        description: "The end-day values and carried balance were updated.",
        destructive: false,
      };
    case "already_initialized":
      return {
        title: "Balance already locked",
        description:
          "This waiter's one-time opening balance cannot be changed.",
        destructive: true,
      };
    case "invalid_initial_balance":
      return {
        title: "Invalid opening balance",
        description: "Enter zero or a negative amount.",
        destructive: true,
      };
    case "invalid_settlement":
      return {
        title: "Invalid settlement",
        description: "Sales and end-day amount must be nonnegative numbers.",
        destructive: true,
      };
    case "initialization_required":
      return {
        ...
        description:
          "Lock this waiter's one-time opening balance before closing a day.",
        destructive: true,
      };
    case "future_date":
      return {
        ...
        description:
          "Only completed or current POS business days can be edited.",
        destructive: true,
      };
    case "before_activation":
      return {
        title: "Date unavailable",
        description: "The waiter ledger begins on July 1, 2026.",
        destructive: true,
      };
    case "waiter_not_found":
      return {
        title: "Waiter not found",
        description: "The selected staff record is no longer a waiter.",
        destructive: true,
      };
    case "save_failed":
      return {
        title: "Could not save",
        description: "No balance changes were applied. Please try again.",
        destructive: true,
      };
    case "inactive_waiter":
      return {
        ...
        description:
          "Inactive waiters can only correct a closed historical settlement.",
        destructive: true,
      };
    default:
      return null;
        ...
        {rows.map((row) => {
          ...
          return (
            <TableRow key={`${businessDate}-${row.waiterId}`}>
              ...
              <TableCell
                className={
                  row.openingBalance && row.openingBalance < 0
              ...
              <TableCell>
                <Label
                  className="sr-only"
                  htmlFor={`sales-${businessDate}-${row.waiterId}`}
                >
              ...
              <TableCell className="font-medium">
                {formatMoney(row.posSales)}
              </TableCell>
              <TableCell>
                <Label
                  className="sr-only"
                  htmlFor={`end-day-${businessDate}-${row.waiterId}`}
                >
              ...
              <TableCell
                className={
                  row.dailyDifference != null && row.dailyDifference < 0
                    ? "font-semibold text-destructive"
                    : ""
                }
              >
              ...
              <TableCell
                className={
                  row.endingBalance != null && row.endingBalance < 0
                    ? "font-semibold text-destructive"
                    : "font-semibold"
                }
              >
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {showInactive ? (
                      <p className="max-w-44 text-xs text-muted-foreground">
                        Historical corrections are only allowed when a closed
    ...
    return (
      <div className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
        ...
        <Alert>
          ...
          <AlertDescription>
            Existing shifts remain unchanged. At activation, each waiter
            receives one immutable opening balance.
          </AlertDescription>
        </Alert>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          />
          <MetricCard
            label="Starting business day"
            value="Jul 1"
            helper="POS window: 7:00 AM to 5:00 AM"
          />
        </div>
      </div>
    );
    ...
    <div className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
      ...
      {invalidDateRequested ? (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Showing the most recently completed business day instead. The active
            POS business day becomes available after it closes at 5:00 AM.
          </AlertDescription>
        </Alert>
      ) : null}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {formatBusinessDay(selectedBusinessDate)} This page defaults to the
            most recently completed POS business day.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          label="Initialized waiters"
          value={`${initializedCount}/${rows.length}`}
          helper="One-time opening balances locked"
        />
        <MetricCard
          label="Closed records"
          value={`${recordedCount}/${rows.length}`}
          helper="Selected business day"
        />
        <MetricCard
          label="POS reference sales"
          value={formatMoney(posSalesTotal)}
          helper="Reference only; manual sales drives debt"
        />
        <MetricCard
          label="Outstanding balance"
          value={formatMoney(outstandingBalance)}
          helper="Negative shortages still owed"
        />
      </div>
      ...
      {initializedCount < rows.length ? (
        <Alert>
          ...
          <AlertTitle>
            {rows.length - initializedCount} waiter(s) need an opening balance
          </AlertTitle>
          <AlertDescription>
            Lock zero or a negative starting amount once. It cannot be edited
            afterward.
          </AlertDescription>
        </Alert>
      ) : null}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manual sales controls the balance calculation. POS sales is
            displayed only to help detect entry mistakes.
          </CardDescription>
        </CardHeader>
