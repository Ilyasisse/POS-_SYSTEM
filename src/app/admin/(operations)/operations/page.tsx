...
export default async function OperationsPage() {
  ...
  const [targets, incidents, templates, runs, staff] = await Promise.all([
    ...
    prisma.cleaningChecklistTemplate.findMany({
      where: { isActive: true },
      include: { tasks: true },
      orderBy: { name: "asc" },
    }),
    prisma.cleaningChecklistRun.findMany({
  ...
  const canManageIncidents = hasPermission(
    user,
    ...
    >
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {KITCHEN_STATIONS.map((station) => {
              const current = targets.find(
                (target) => target.station === station,
              );
              return (
                <form
                  key={station}
                  action={saveKitchenTargetAction}
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      min="1"
                      max="240"
                      defaultValue={current?.targetMinutes ?? 15}
                      aria-label={`${station} target minutes`}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="space-y-4 p-5">
          ...
          {canManageCleaning ? (
            <form action={createCleaningTemplateAction} className="grid gap-3">
              <Input name="name" placeholder="Checklist name" required />
              <select
                name="station"
                className={fieldClass}
                aria-label="Cleaning station"
              >
                <option value="">All areas</option>
                {KITCHEN_STATIONS.map((station) => (
                  <option key={station}>{station}</option>
              ...
              <Input
                name="schedule"
                placeholder="Schedule, e.g. Daily at 17:00"
                required
              />
              <textarea
                name="tasks"
                className="min-h-28 rounded-lg border p-3"
                placeholder="One required task per line"
                aria-label="Cleaning checklist tasks"
                required
              />
          ...
          {templates.map((template) => (
            <div key={template.id} className="rounded-xl border p-3">
              ...
              {canManageCleaning ? (
                <form
                  action={scheduleCleaningRunAction}
                ...
                >
                  ...
                  <select
                    name="assignedToUserId"
                    className={fieldClass}
                    aria-label="Assign cleaning run"
                  >
                    <option value="">Unassigned</option>
                    {staff.map((member) => (
                      <option key={member.id} value={member.id}>
            ...
            runs.map((run) => (
              <div key={run.id} className="rounded-xl border p-3">
                ...
                <p className="text-xs text-slate-500">
                  ...
                  {run.assignedTo ? ` · ${run.assignedTo.fullName}` : ""}
                </p>
                <div className="mt-3 space-y-2">
                  {run.tasks.map((line) => (
                    <form
                      key={line.id}
                      action={completeCleaningTaskAction}
                      className="flex items-center gap-2"
                    >
                      <input type="hidden" name="runTaskId" value={line.id} />
                      <span className={line.completed ? "line-through" : ""}>
                        {line.task.label}
                      </span>
                      {!line.completed ? (
                        <>
                ...
                <form
                  action={completeCleaningRunAction}
                  className="mt-3 grid gap-2"
                >
                  <input type="hidden" name="runId" value={run.id} />
                  <Input
                    name="evidenceNote"
                    placeholder="Completion evidence note"
                  />
                  <Input
                    name="evidenceUrl"
                    type="url"
                  ...
                  />
                  <Button className="w-fit">Complete run</Button>
                </form>
              </div>
            ))
          )}
