...
export function SupplyEntryFields({
  ...
  const [currentUnitPrice, setCurrentUnitPrice] = useState(
    unitPrice ||
      initialItem?.defaultUnitPrice ||
      catalogItems[0]?.defaultUnitPrice ||
      "",
  );
  const [priceOverride, setPriceOverride] = useState(
    Boolean(
      unitPrice && initialItem && unitPrice !== initialItem.defaultUnitPrice,
    ),
  );
  const selectedItem = catalogItems.find(
    (item) => item.id === currentCatalogItemId,
  );
...
export default function SupplyEntryForm({
  ...
  return (
    <form
      action={action}
      className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1.5fr_0.7fr_0.8fr_0.8fr_auto] xl:items-end"
    >
      <Input type="hidden" name="purchaseDate" value={purchaseDate} />
      <SupplyEntryFields prefix="new-supply" catalogItems={catalogItems} />
      <Button type="submit" disabled={catalogItems.length === 0}>
        Add item
      </Button>
    </form>
  );
