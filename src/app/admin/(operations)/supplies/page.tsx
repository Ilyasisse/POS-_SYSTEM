...
export default async function SuppliesPage({ searchParams }: SupplyPageProps) {
  ...
  const [entries, historyEntries, day, catalogRows] = await Promise.all([
    ...
    prisma.supplyDay.findUnique({
      where: { purchaseDate: selectedDatabaseDate },
      include: { _count: { select: { payments: true } } },
    }),
    prisma.supplyCatalogItem.findMany({
  ...
  const activeCatalogItems = catalogItems.filter(
    (_, index) => catalogRows[index].isActive,
  );
      ...
      <Card className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="font-black">
            {closed ? "Supply day closed" : "Supply day open"}
          </p>
          <p className="text-sm text-muted-foreground">
            {closed
              ? `Payable from ${displayDate(shiftDate(selectedDate, 1))}.`
              : "Add every supply received, then close the day."}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {closed ? (
            <form action={reopenSupplyDay}>
              <input type="hidden" name="date" value={selectedDate} />
              <Button
                variant="outline"
                disabled={(day?._count.payments ?? 0) > 0}
              >
                Reopen day
              </Button>
            </form>
          ) : (
            <form action={closeSupplyDay}>
              <input type="hidden" name="date" value={selectedDate} />
              <Button disabled={entries.length === 0}>Close supply day</Button>
            </form>
          )}
        </div>
      </Card>
        ...
        <Card className="p-5">
          ...
          {activeCatalogItems.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">
              Create an active supply item before adding today&apos;s purchases.
            </p>
          ) : null}
        </Card>
      ) : null}
