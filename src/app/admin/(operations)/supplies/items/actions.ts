...
import { parseSupplyUnitPrice } from "@/lib/supplies/supply-purchases";
...
function values(formData: FormData) {
  ...
  const price = parseSupplyUnitPrice(priceText);
  if (
    !name ||
    name.length > 160 ||
    !unit ||
  ...
  return { name, unit, defaultUnitPrice: price };
}
...
export async function createSupplyCatalogItem(formData: FormData) {
  ...
  await prisma.supplyCatalogItem.create({
    data: { ...values(formData), createdByUserId: user.id },
  });
  refresh();
}
...
export async function setSupplyCatalogItemActive(formData: FormData) {
  ...
  await prisma.supplyCatalogItem.update({
    where: { id },
    data: { isActive: field(formData, "active") === "true" },
  });
  refresh();
}
