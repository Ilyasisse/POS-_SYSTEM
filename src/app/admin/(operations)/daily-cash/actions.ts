...
import {
  createDailyCashSavingsDeposit,
  createManualExpense,
  deleteManualExpense,
  finalizeDailyCash,
  overrideDailySalary,
  payDailyCashObligation,
  payDailyCashSupplierAdvance,
  payDailyCashSupply,
  payDailySalary,
  undoDailyCashSavingsDeposit,
  undoDailyCashSupplierPayment,
  undoDailyCashSupplyPayment,
  undoDailySalary,
...
export async function overrideSalaryAction(formData: FormData) {
  ...
  await overrideDailySalary({
    dateKey: text(formData, "date"),
...
export async function paySalaryAction(formData: FormData) {
  const user = await requirePermission(PERMISSIONS.DAILY_CASH_MANAGE);
  const result = await payDailySalary({
    dateKey: text(formData, "date"),
    userId: user.id,
    confirmSavings: formData.get("confirmSavings") === "on",
  });
  if (!result.ok)
    throw new Error(
      result.code === "SAVINGS_CONFIRMATION_REQUIRED"
        ? `This payment needs $${result.savingsAmount} from savings. Tick the confirmation box and submit again.`
        : result.message,
    );
  refresh();
}
export async function createManualExpenseAction(formData: FormData) {
  ...
  if (!result.ok)
    throw new Error(
      result.code === "SAVINGS_CONFIRMATION_REQUIRED"
        ? `This expense needs $${result.savingsAmount} from savings. Tick the confirmation box and submit again.`
        : result.message,
    );
  refresh();
}
...
export async function createSavingsDepositAction(formData: FormData) {
  const user = await requirePermission(PERMISSIONS.DAILY_CASH_MANAGE);
  const result = await createDailyCashSavingsDeposit({
    dateKey: text(formData, "date"),
    amount: money(formData, "amount"),
    note: text(formData, "note"),
    userId: user.id,
  ...
  if (!result.ok)
    throw new Error(
      "message" in result ? result.message : "Unable to move cash to savings.",
    );
  refresh();
}
export async function undoSavingsDepositAction(formData: FormData) {
  await requirePermission(PERMISSIONS.DAILY_CASH_MANAGE);
  await undoDailyCashSavingsDeposit({
    dateKey: text(formData, "date"),
    id: text(formData, "id"),
  });
  refresh();
}
export async function undoPaidActivityAction(input: {
  date: string;
  ...
  if (!input.rowId.startsWith(prefix))
    throw new Error("Invalid paid activity.");
...
export async function paySupplyBalanceAction(formData: FormData) {
  const user = await requirePermission(PERMISSIONS.DAILY_CASH_MANAGE);
  const result = await payDailyCashSupply({
    dateKey: text(formData, "date"),
    supplyDayId: text(formData, "supplyDayId"),
    amount: money(formData, "amount"),
    userId: user.id,
    confirmSavings: formData.get("confirmSavings") === "on",
  });
...
export async function paySupplierObligationAction(formData: FormData) {
  const user = await requirePermission(PERMISSIONS.DAILY_CASH_MANAGE);
  const result = await payDailyCashObligation({
    dateKey: text(formData, "date"),
    billId: text(formData, "billId"),
    installmentId: text(formData, "installmentId") || null,
    amount: money(formData, "amount"),
    userId: user.id,
    confirmSavings: formData.get("confirmSavings") === "on",
...
export async function paySupplierAdvanceAction(formData: FormData) {
  const user = await requirePermission(PERMISSIONS.DAILY_CASH_MANAGE);
  const result = await payDailyCashSupplierAdvance({
    dateKey: text(formData, "date"),
