...
export async function POST(request: Request) {
  ...
  if (
    !report ||
    !formats.has(format as ExportFormat) ||
    !reports.has(report as ExportReport)
  )
    return NextResponse.json(
      { error: "report and format=csv|xlsx|pdf|print are required." },
      { status: 400 },
    );
  ...
  if (selectedFormat === "print")
    return new NextResponse(toPrintHtml(`${report} report`, rows), {
      headers: { "Content-Type": "text/html; charset=utf-8" },
    });
  if (selectedFormat === "xlsx") {
    const file = await toXlsx(`${report} report`, rows);
  ...
  if (selectedFormat === "pdf") {
    const file = await toPdf(`${report} report`, rows);
    return new NextResponse(new Uint8Array(file), {
      headers: {
