...
import { getOrderTypeReport } from "@/lib/reports/services/order-type-report-service";
import { reportQuerySchema } from "@/lib/reports/validation";

export const dynamic = "force-dynamic";

export async function GET(request: Request) {
  const authorization = await authorizeApi(PERMISSIONS.REPORT_DAILY_VIEW);
  ...
  try {
    return NextResponse.json(
      await getOrderTypeReport(resolveReportRange(parsed.data), parsed.data),
    );
  } catch (error) {
