...
export default async function ManagerPage({ searchParams }: ManagerPageProps) {
  ...
  const [waiters, openTableOrders] = await Promise.all([
  ...
  const alerts: Alert[] = [
    ...openTableOrders
      .filter((order) => currentTime - order.createdAt.getTime() > 60 * 60 * 1000)
      .map((order) => ({
    ...
    ...summaries
      .filter((summary) => summary.hasClosedShift && summary.hasOrdersWithoutPayments)
      .map((summary) => ({
        tone: "warning" as const,
    ...
    <main className="p-6">
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <h2 className="mt-2 text-2xl font-bold">{summaries.length}</h2>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <h2 className="mt-2 text-2xl font-bold">{formatMoney(grandTotalSales)}</h2>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-sm text-slate-500">Open table total</p>
          <h2 className="mt-2 text-2xl font-bold">{formatMoney(openTableTotal)}</h2>
        </div>
      </div>
      ...
      <div className="mb-6 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <form action={saveWaiterOpeningBalance} className="rounded-xl border border-slate-200 p-4">
              <input type="hidden" name="waiterId" value={selectedWaiterId} />
              <input type="hidden" name="openingAmount" value={openingAmountDefaultValue.toFixed(2)} />
              <span className="mb-1 block text-sm font-medium text-slate-700">
                Starting balance
              </span>
              ...
              <button
                type="submit"
                disabled={!selectedWaiterId || Boolean(selectedWaiterIsClosed) || Boolean(selectedWaiterHasOpenShift)}
            ...
            <form action={closeWaiterBalanceFromManager} className="rounded-xl border border-slate-200 p-4">
              <input type="hidden" name="waiterId" value={selectedWaiterId} />
              <label className="block">
                <span className="mb-1 block text-sm font-medium text-slate-700">
                  Closing balance
          ...
          {selectedWaiterSummary ? (
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl bg-slate-50 p-3">
                <p className="text-xs uppercase tracking-[0.15em] text-slate-500">Waiter</p>
                <p className="mt-1 font-semibold text-slate-900">{selectedWaiterSummary.fullName}</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-3">
                <p className="text-xs uppercase tracking-[0.15em] text-slate-500">Status</p>
                <p className="mt-1 font-semibold capitalize text-slate-900">{selectedWaiterSummary.shiftSummary.status}</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-3">
                <p className="text-xs uppercase tracking-[0.15em] text-slate-500">Expected close</p>
                <p className="mt-1 font-semibold text-slate-900">{formatMoney(selectedWaiterSummary.shiftSummary.expectedClosingAmount)}</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-3">
                <p className="text-xs uppercase tracking-[0.15em] text-slate-500">Difference</p>
      ...
      <section className="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {openTableOrders.length === 0 ? (
          <div className="p-6 text-sm text-slate-500">No unpaid table orders.</div>
        ) : (
              ...
              <tbody>
                {openTableOrders.map((order) => (
                  <tr key={order.id} className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-3 text-slate-600">{order.cashier?.fullName ?? "-"}</td>
                    <td className="px-4 py-3 text-slate-600">
                      {order.orderItems.map((item) => `${item.qty}x ${item.productName}`).join(", ")}
                    </td>
                    <td className="px-4 py-3 font-semibold">{formatMoney(Number(order.total))}</td>
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-slate-700">Opening</th>
            ...
            <tbody>
              {summaries.map((staff) => (
                <tr key={staff.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3 capitalize">{staff.shiftSummary.status}</td>
                  <td className={`px-4 py-3 ${
                    staff.shiftSummary.variance != null && staff.shiftSummary.variance < 0
