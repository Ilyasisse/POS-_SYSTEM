  ...
  const [shifts, events] = await Promise.all([
    prisma.scheduledShift.findMany({
      where: {
        workerId: worker.id,
        status: "SCHEDULED",
        endsAt: { gte: new Date(now.getTime() - 12 * 3_600_000) },
      },
      ...
      <Card className="p-5">
        ...
        >
          ...
          <Button type="submit" className="self-end">
            Clock {nextType === "IN" ? "in" : "out"}
          </Button>
        </form>
      </Card>
      <section className="grid gap-4 md:grid-cols-2">
        <Card className="p-5">
          <h2 className="font-bold">Upcoming shifts</h2>
        ...
        <Card className="p-5">
          <h2 className="font-bold">Recent clock events</h2>
