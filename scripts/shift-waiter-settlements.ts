...
function usage(message?: string): never {
  ...
  console.error(
    "Usage: npm run waiter-balances:shift-date -- --date=YYYY-MM-DD [--apply]",
  );
  process.exit(1);
}
...
if (
  unknownArgument ||
  !dateArgument ||
  args.filter((argument) => argument.startsWith("--date=")).length !== 1
) {
  usage(
    unknownArgument
      ? `Unknown option: ${unknownArgument}`
      : "Provide exactly one --date option.",
  );
}
...
async function main() {
  try {
    ...
    console.log(
      `Incomplete source records: ${result.incompleteShiftIds.length}`,
    );
    ...
    if (result.incompleteShiftIds.length || result.conflictingShiftIds.length) {
      ...
      console.log(
        `Committed: moved ${result.candidateShiftIds.length} settlement(s).`,
      );
    } else {
      console.log(
        "Dry run only. Re-run with --apply after reviewing this result.",
      );
    }
  } catch (error) {
    console.error(
      error instanceof Error ? error.message : "Settlement shift failed.",
    );
    process.exitCode = 1;
  }
}
